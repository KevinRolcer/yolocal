...
$preferencia = null;
$redirigirPago = '';
$linkPagoManual = 'https://mpago.la/1MLH1oE';
$modoApiMercadoPago = mercadoPagoApiDisponible();
$usuarioId = (int)($_SESSION['ID_Usuario'] ?? $_SESSION['id'] ?? 0);
$pagoNormalVigente = $usuarioId > 0 && usuarioTienePagoNormalActivo($usuarioId);
...
$debugVersionTag = 'YOLOCAL-DEPLOY-2026-07-31-PAGOS';
$mostrarConfirmacion = (int)($_SESSION['mostrar_confirmacion_pago_normal'] ?? 0) === 1 || $forzarConfirmacion || $pagoNormalVigente;
$confirmacionPagoValidada = (int)($_SESSION['confirmacion_pago_normal_validada'] ?? 0) === 1 || $pagoNormalVigente;
...
if ($estadoAprobado) {
    ...
    if ($usuarioId > 0) {
        $actualizacionOk = actualizarPagoPorRetornoMercadoPago(
            ...
            $referenciaRetorno,
            [
                'filtro_init_point' => $modoApiMercadoPago ? '' : $linkPagoManual,
    ...
    $_SESSION['mostrar_confirmacion_pago_normal'] = 1;
    $_SESSION['confirmacion_pago_normal_validada'] = $actualizacionOk ? 1 : 0;
    $mostrarConfirmacion = true;
    $confirmacionPagoValidada = $actualizacionOk;

    if (!isset($_GET['confirmacion'])) {
        header('Location: ' . $baseUrl . 'index.php?pag=pagos&confirmacion=1');
        exit();
    }
}

if ($mostrarConfirmacion) {
    unset($_SESSION['mostrar_confirmacion_pago_normal']);
    unset($_SESSION['confirmacion_pago_normal_validada']);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['crear_pago'])) {
    ...
    if ($referenciaPago === '') {
        $referenciaPago = generarReferenciaPago($usuarioId, $diaPago) . '-PN';
    } elseif (strpos($referenciaPago, '-PN') === false) {
        $referenciaPago .= '-PN';
    }
    if ($diaPago === '') {
    ...
    $titulo = 'Pago YoLocal - ' . $nombreDueno;
    $descripcion = 'Local: ' . $nombreLocal . ' | Referencia: ' . $referenciaPago . ' | Fecha: ' . $diaPago;
    $email = trim($_POST['email'] ?? '');

    if ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $mensaje = 'Ingresa un correo valido para continuar con tu pago.';
    }
    ...
    if ($mensaje === '') {
        $preferenciaId = '';
        $linkCheckout = $linkPagoManual;

        if ($modoApiMercadoPago) {
            $retorno = obtenerUrlRetornoPago('index.php?pag=pagos');
        ...
        if ($linkCheckout === '') {
            $mensaje = 'No se pudo obtener el enlace de pago.';
        }

        // Guardar registro pendiente antes de redirigir
        if ($mensaje === '') {
            $datosSave = [
            ...
            if (guardarPago($datosSave)) {
                $redirigirPago = $linkCheckout;
            } else {
                $mensaje = 'No se pudo registrar tu pago pendiente. Intenta nuevamente.';
            }
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .btn-pagar:hover  { background: var(--principal2); transform: translateY(-1px); }
        ...
        .confirmacion-card {
            ...
            border-radius: 26px;
            padding: 42px 34px;
            box-shadow: 0 26px 80px rgba(91, 33, 182, 0.2);
            text-align: center;
            animation: confirmPopIn .26s ease-out;
        }
        ...
        .confirmacion-icon {
            ...
            border-radius: 50%;
            margin: 0 auto 20px;
            background: linear-gradient(135deg, #4c0682, #f8c300);
            color: #fff;
            display: inline-flex;
            align-items: center;
            justify-content: center;
        ...
        .confirmacion-btn-main,
        .confirmacion-btn-alt {
            display: inline-flex;
            align-items: center;
            justify-content: center;
            gap: 8px;
...
<body>
        ...
        <div class="content p-4">
            ...
            <?php if (!$mostrarConfirmacion): ?>
                ...
                <div class="pago-header">
                    <div class="icono-mp"><i class="bi bi-credit-card-2-front-fill"></i></div>
                    <div>
                        <h2><b>Mantén activa tu participación en Yo Local</b></h2>
                        <p>Gracias por seguir sumando a nuestra comunidad <span class="cuadro-icono">💜</span></p>
                    </div>
                </div>
                ...
                <div class="pago-layout">
                    <div class="pago-card">
                        ...
                        <div class="row g-3 mb-2">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="nombre_dueno" class="form-control"
                                       value="<?= htmlspecialchars($datosPago['nombre_dueno']) ?>" readonly>
                            </div>
                            <div class="col-md-6">
                                ...
                                <?php if (!empty($datosPago['negocios'])): ?>
                                    <select name="nombre_local" class="form-select" required>
                                        <?php foreach ($datosPago['negocios'] as $negocio): ?>
                                            <option value="<?= htmlspecialchars($negocio['nombre_negocio']) ?>"
                                                <?= $negocio['nombre_negocio'] === $datosPago['nombre_local'] ? 'selected' : '' ?>>
                                                <?= htmlspecialchars($negocio['nombre_negocio']) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                <?php else: ?>
                                    <input type="text" name="nombre_local" class="form-control"
                                           value="<?= htmlspecialchars($datosPago['nombre_local']) ?>" readonly>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <!-- Datos del pago -->
                        <p class="fw-semibold mb-3" style="color:var(--principal); font-size:13px; text-transform:uppercase; letter-spacing:.6px;">
                            <i class="bi bi-receipt me-1"></i> Datos de tu pago
                        </p>
                        <div class="row g-3 mb-2">
                            <div class="col-md-6">
                                <label class="form-label">Referencia de pago</label>
                                <input type="text" name="referencia_pago" class="form-control"
                                       value="<?= htmlspecialchars($datosPago['referencia_pago'] ?: generarReferenciaPago($usuarioId, $datosPago['dia_pago'])) ?>" readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Fecha de pago</label>
                                <input type="date" name="dia_pago" class="form-control"
                                       value="<?= htmlspecialchars($datosPago['dia_pago']) ?>" readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Correo electronico <span class="text-danger">*</span></label>
                                <input type="email" name="email" class="form-control" value="<?= htmlspecialchars($datosPago['email']) ?>" placeholder="usuario@correo.com" required>
                            </div>
                        </div>
                        ...
                        <div class="pago-aviso">
                            ...
                            Tus pagos se gestionan de forma segura. Al confirmar, tu pago quedara registrado.
                        </div>
                    ...
                    <figure class="pago-side-image">
                        <img src="<?= $baseUrl ?>assets/img/banners/bannerdepagonormal.png" alt="Banner lateral de pago normal">
                    </figure>
                </div>
            <?php else: ?>
                <div class="confirmacion-wrap" role="dialog" aria-modal="true" aria-label="Confirmacion de pago">
                    <div class="confirmacion-card">
                        <div class="confirmacion-icon"><i class="bi bi-heart-fill"></i></div>
                        <h2 class="confirmacion-title">¡Gracias por seguir creyendo en lo local! 💜</h2>
                        <p class="confirmacion-text">Gracias por tu confianza. Sigamos impulsando juntos a los negocios locales.</p>
                        <p class="confirmacion-status"><i class="bi bi-check-circle-fill" style="color:#22c55e;margin-right:6px;"></i>Tu pago ha sido procesado correctamente.</p>
                        <p class="confirmacion-status"><i class="bi bi-repeat" style="color:#3b82f6;margin-right:6px;"></i>Tus pagos se procesaran automaticamente cada mes.</p>
                        <div class="confirmacion-actions">
                            <a href="<?= $baseUrl ?>index.php?pag=home" class="confirmacion-btn-main"><i class="bi bi-house"></i> Ir al inicio</a>
                            <a href="<?= $baseUrl ?>index.php?pag=pagos_usuario" class="confirmacion-btn-alt"><i class="bi bi-wallet2"></i> Ver mis pagos</a>
                        </div>
                    </div>
                </div>
                <figure class="confirmacion-beneficios" aria-label="Beneficios para negocios">
                    <img src="<?= $baseUrl ?>assets/img/banners/bannerdepagonormal.png" alt="Beneficios de mantener activo tu pago en Yo Local" class="js-expandible-beneficios">
                </figure>
            <?php endif; ?>
        </div>
    </div>

    <div id="lightboxBeneficiosPagos" class="image-lightbox" role="dialog" aria-modal="true" aria-label="Imagen ampliada de beneficios">
        <div class="image-lightbox-controls" aria-label="Controles de zoom">
            <button type="button" id="zoomOutPagos" class="image-lightbox-btn" aria-label="Reducir imagen">-</button>
            <button type="button" id="zoomResetPagos" class="image-lightbox-btn" aria-label="Restablecer zoom">•</button>
            <button type="button" id="zoomInPagos" class="image-lightbox-btn" aria-label="Aumentar imagen">+</button>
            <span id="zoomNivelPagos" class="image-lightbox-zoom">100%</span>
            <button type="button" id="zoomClosePagos" class="image-lightbox-btn" aria-label="Cerrar imagen">×</button>
        </div>
    ...
    (function () {
        ...
        var lightbox = document.getElementById('lightboxBeneficiosPagos');
        var lightboxImg = document.getElementById('lightboxBeneficiosPagosImg');
        var zoomOut = document.getElementById('zoomOutPagos');
        var zoomIn = document.getElementById('zoomInPagos');
        var zoomReset = document.getElementById('zoomResetPagos');
        var zoomClose = document.getElementById('zoomClosePagos');
        var zoomNivel = document.getElementById('zoomNivelPagos');
        var escala = 1;
        var minZoom = 0.6;
        var maxZoom = 3;
        var posX = 0;
